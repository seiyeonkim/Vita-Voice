  ...
  useFocusEffect(
    useCallback(() => {
      (async () => {
        try {
          // ✅ 서버에서 기록 불러오기
          const serverRecords = await fetchHistoryList();
  
          // ✅ 로컬에 저장 (선택사항: 캐시 목적)
          await AsyncStorage.setItem('@history', JSON.stringify(serverRecords));
  
          // ✅ 화면에 표시할 상태 반영
          setRecords(serverRecords);
        } catch (err) {
          console.error('서버에서 진단 기록을 불러오지 못했습니다. → 로컬 캐시 fallback');
